...
public class CalendarAdapter extends BaseAdapter {
	...
	String tdy_day,grid_day;
	private ArrayList<String> items;
	public static List<String> dayString;
	private View previousView;
	View v;

	String[] today_array,cutday_array;
	String today_year, today_mon, today_day,cutday_year, cutday_mon, cutday_day;

	public CalendarAdapter(Context c, GregorianCalendar monthCalendar, String sel_date, String dt, String today_date) {
		CalendarAdapter.dayString = new ArrayList<String>();
		Locale.setDefault(Locale.US);
		this.today_date = today_date;
		if (CalendarView.btn_click != 2) {
			this.today_date = dt;
		} else {
			today_date = "" + CalendarView.cin_year + "-" + CalendarView.cin_mon + "-" + CalendarView.cin_day;
		}
		month = monthCalendar;
		...
		cutday_array = sel_date.split("-");
		cutday_year = cutday_array[0].replaceFirst("^0*", "");
		cutday_mon = cutday_array[1].replaceFirst("^0*", "");
		cutday_day = cutday_array[2].replaceFirst("^0*", "");
		refreshDays();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Date date1;
		try {
			date1 = inFormat.parse("" + cutday_day + "-" + cutday_mon + "-" + cutday_year);
			SimpleDateFormat outFormat = new SimpleDateFormat("EEE");
			 tdy_day = outFormat.format(date1);

			System.out.println("goal_tdyday::" + tdy_day);
		...
		Date date2;
		try {
			date2 = inFormat1.parse("" +gridvalue + "-" + grid_mon + "-" + grid_year);
		...
		if ((Integer.parseInt(gridvalue) > 1) && (position < firstDay)) {
			...
			if (Integer.parseInt(grid_year) == Integer.parseInt(today_year)) {
				...
				if (Integer.parseInt(grid_mon) == Integer.parseInt(today_mon)) {
					if (Integer.parseInt(gridvalue) < Integer.parseInt(today_day)) {
				...
				} else if (Integer.parseInt(grid_mon) < Integer.parseInt(today_mon)) {
					dayView.setTextColor(Color.GRAY);
					ll_date.setBackgroundColor(Color.parseColor("#ededed"));
					dayView.setClickable(true);
					ll_date.setClickable(true);
				...
				if(grid_day.toUpperCase().equalsIgnoreCase(tdy_day))
				{
					dayView.setTextColor(Color.RED);
					if((Integer.parseInt(grid_year)==(Integer.parseInt(today_year))&&(Integer.parseInt(grid_mon)==(Integer.parseInt(today_mon))&&(Integer.parseInt(gridvalue)<(Integer.parseInt(today_day)))))){
						dayView.setTextColor(Color.GRAY);
						ll_date.setBackgroundColor(Color.parseColor("#ededed"));
						dayView.setClickable(true);
						ll_date.setClickable(true);
				...
				if(grid_day.equalsIgnoreCase("Mon"))
				{
					CalendarView.txt_Mon.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Tue"))
				{
					CalendarView.txt_TUE.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Wed"))
				{
					CalendarView.txt_WED.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Thu"))
				{
					CalendarView.txt_THU.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Fri"))
				{
					CalendarView.txt_FRI.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Sat"))
				{
					CalendarView.txt_SAT.setTextColor(Color.RED);
				}
				if(grid_day.equalsIgnoreCase("Sun"))
				{
					CalendarView.txt_SUN.setTextColor(Color.RED);
				}
...
//					dayView.setTextColor(Color.GREEN);
			...
			} else if (Integer.parseInt(grid_year) < Integer.parseInt(today_year)) {
				dayView.setTextColor(Color.GRAY);
				ll_date.setBackgroundColor(Color.parseColor("#ededed"));
				dayView.setClickable(true);
				ll_date.setClickable(true);
			...
			System.out.println( " somedays  are "+grid_day +" - "+tdy_day);
